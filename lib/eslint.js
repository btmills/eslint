...
/**
 * Wraps a syntax tree in node.js' function expression to create scope.
 * @param   g{ASTNode} ast Program to wrap
 * @returns {ASTNode}     Wrapped Program
 */
function wrap(ast) {

    /**
     * Add location information to an AST node
     * @param  {ASTNode} source Optional node from which to copy information
     * @param  {ASTNode} node   Node that needs location properties
     * @return {ASTNode}        Node, now with location information
     */
    function locate(source, node) {
        if (!node) {
            node = source;
            source = {};
        }

        node.range = source.range || [0, 0];
        node.loc = source.loc || {
            start: {
                line: 1,
                column: 0
    ...
    var params = ["exports", "require", "module", "__filename", "__dirname"],
        disableComment = locate({
            type: "Block",
            value: "eslint-disable no-unused-vars, strict "
        }),
        enableComment = locate({
            type: "Block",
            value: "eslint-enable no-unused-vars, strict "
        }),
        firstExpr;
    ...
    // Attach disable/enable comments
    ast.body[0].expression.params[0].leadingComments = [disableComment];
    firstExpr = ast.body[0].expression.body.body[0];
    if (firstExpr) {
        if (firstExpr.leadingComments) {
            firstExpr.leadingComments.unshift(enableComment);
        } else {
            firstExpr.leadingComments = [enableComment];
        }
    }
    ast.comments.unshift(disableComment, enableComment);
...
module.exports = (function() {
    ...
    api.verify = function(text, config, filename, saveState) {
        ...
        if (ast) {
            ...
            wrappedAst = config.env.node ? wrap(ast) : ast;

            // parse global comments and modify config
            modifyConfigsFromComments(wrappedAst, config, reportingConfig);
            ...
            currentScopes = escope.analyze(wrappedAst, { ignoreEval: true }).scopes;
            ...
            addDeclaredGlobals(wrappedAst, currentScopes[0], currentConfig);
